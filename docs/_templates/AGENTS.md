...
```python
def create_ templates(config: Dict[str, Any]) ->  Templates:
    """Create  Templates using factory pattern with validation"""

    # Validate configuration
    validate_ templates_config(config)
...
```python
def test_ templates_initialization():
    ...
    component = create_ templates(config.to_dict())

    # Validate initialization
    assert component.config == config.to_dict()
    assert component.initialized == True

def test_ templates_functionality():
