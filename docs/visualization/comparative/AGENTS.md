# Comparative - Agent Development Guide

**Guidelines for AI agents working with Comparative in the Active Inference Knowledge Environment.**

**"Active Inference for, with, by Generative AI"**

## 🤖 Agent Role & Responsibilities

**What agents should do when working with Comparative:**
...
**Understanding how Comparative fits into the platform:**

```
Platform Layer
├── Knowledge Layer (foundations/, mathematics/, implementations/)
├── Platform Layer ← Comparative
└── Integration Layer (platform/, visualization/, tools/)
```

### Integration Points

**Comparative integrates with multiple platform components:**
...
**All visualization_framework development must follow these patterns:**

#### 1. Component Factory Pattern (PREFERRED)
```python
def create_comparative(config: Dict[str, Any]) -> Comparative:
    """Create Comparative using factory pattern with validation"""

    # Validate configuration
    validate_comparative_config(config)
...
```python
def test_comparative_initialization():
    """Test Comparative initialization"""
    config = ComparativeConfig(
        component_name="Comparative",
        config_field="test_value"
    )

    component = create_comparative(config.to_dict())

    # Validate initialization
    assert component.config == config.to_dict()
    assert component.initialized == True

def test_comparative_functionality():
    """Test core Comparative functionality"""
    config = ComparativeConfig(
        component_name="Comparative",
        config_field="test_value"
    )
...
component = Comparative(debug_config)
```
...
**Component**: Comparative | **Version**: 1.0.0 | **Last Updated**: October 2024
