# Benchmarks - Agent Development Guide

**Guidelines for AI agents working with Benchmarks in the Active Inference Knowledge Environment.**

**"Active Inference for, with, by Generative AI"**

## 🤖 Agent Role & Responsibilities

**What agents should do when working with Benchmarks:**
...
**Understanding how Benchmarks fits into the platform:**

```
Platform Layer
├── Knowledge Layer (foundations/, mathematics/, implementations/)
├── Platform Layer ← Benchmarks
└── Integration Layer (platform/, visualization/, tools/)
```

### Integration Points

**Benchmarks integrates with multiple platform components:**
...
**All research_benchmarking development must follow these patterns:**

#### 1. Component Factory Pattern (PREFERRED)
```python
def create_benchmarks(config: Dict[str, Any]) -> Benchmarks:
    """Create Benchmarks using factory pattern with validation"""

    # Validate configuration
    validate_benchmarks_config(config)

    # Create component with validation
    component = Benchmarks(config)
...
```python
def test_benchmarks_initialization():
    """Test Benchmarks initialization"""
    config = BenchmarksConfig(
        component_name="Benchmarks",
        config_field="test_value"
    )

    component = create_benchmarks(config.to_dict())

    # Validate initialization
    assert component.config == config.to_dict()
    assert component.initialized == True

def test_benchmarks_functionality():
    """Test core Benchmarks functionality"""
    config = BenchmarksConfig(
        component_name="Benchmarks",
        config_field="test_value"
    )
...
component = Benchmarks(debug_config)
```
...
**Component**: Benchmarks | **Version**: 1.0.0 | **Last Updated**: October 2024
