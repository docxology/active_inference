# Dashboards - Agent Development Guide

**Guidelines for AI agents working with Dashboards in the Active Inference Knowledge Environment.**

**"Active Inference for, with, by Generative AI"**

## 🤖 Agent Role & Responsibilities

**What agents should do when working with Dashboards:**
...
**Understanding how Dashboards fits into the platform:**

```
Platform Layer
├── Knowledge Layer (foundations/, mathematics/, implementations/)
├── Platform Layer ← Dashboards
└── Integration Layer (platform/, visualization/, tools/)
```

### Integration Points

**Dashboards integrates with multiple platform components:**
...
**All visualization_dashboards development must follow these patterns:**

#### 1. Component Factory Pattern (PREFERRED)
```python
def create_dashboards(config: Dict[str, Any]) -> Dashboards:
    """Create Dashboards using factory pattern with validation"""

    # Validate configuration
    validate_dashboards_config(config)
...
```python
def test_dashboards_initialization():
    """Test Dashboards initialization"""
    config = DashboardsConfig(
        component_name="Dashboards",
        config_field="test_value"
    )

    component = create_dashboards(config.to_dict())

    # Validate initialization
    assert component.config == config.to_dict()
    assert component.initialized == True

def test_dashboards_functionality():
    """Test core Dashboards functionality"""
    config = DashboardsConfig(
        component_name="Dashboards",
        config_field="test_value"
    )
...
component = Dashboards(debug_config)
```
...
**Component**: Dashboards | **Version**: 1.0.0 | **Last Updated**: October 2024
