# Education - Agent Development Guide

**Guidelines for AI agents working with Education in the Active Inference Knowledge Environment.**

**"Active Inference for, with, by Generative AI"**

## 🤖 Agent Role & Responsibilities

**What agents should do when working with Education:**
...
**Understanding how Education fits into the platform:**

```
Platform Layer
├── Knowledge Layer (foundations/, mathematics/, implementations/)
├── Platform Layer ← Education
└── Integration Layer (platform/, visualization/, tools/)
```

### Integration Points

**Education integrates with multiple platform components:**
...
**All application_education development must follow these patterns:**

#### 1. Component Factory Pattern (PREFERRED)
```python
def create_education(config: Dict[str, Any]) -> Education:
    """Create Education using factory pattern with validation"""

    # Validate configuration
    validate_education_config(config)

    # Create component with validation
    component = Education(config)
...
```python
def test_education_initialization():
    """Test Education initialization"""
    config = EducationConfig(
        component_name="Education",
        config_field="test_value"
    )

    component = create_education(config.to_dict())
...
def test_education_functionality():
    """Test core Education functionality"""
    config = EducationConfig(
        component_name="Education",
        config_field="test_value"
    )
...
component = Education(debug_config)
```
...
**Component**: Education | **Version**: 1.0.0 | **Last Updated**: October 2024
